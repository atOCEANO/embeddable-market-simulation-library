...
def test_tune_is_reproducible_with_a_seed_and_one_job():
    ...
    a = emsl.tune(SmaCross, SPACE, series(), **kwargs)
    b = emsl.tune(SmaCross, SPACE, series(), **kwargs)
...
def test_best_strategy_builds_a_strategy_from_the_best_params():
    result = emsl.tune(SmaCross, SPACE, series(), n_trials=8, seed=0, fee_taker=0.0, fee_maker=0.0)
...
def test_unknown_objective_key_raises_before_the_search():
    ...
    with pytest.raises(KeyError):
        emsl.tune(SmaCross, SPACE, series(), objective="not_a_stat", n_trials=5)
...
def test_misspelled_space_name_raises_before_the_search():
    ...
    with pytest.raises(TypeError):
        emsl.tune(SmaCross, {"fst": (3, 20), "slow": (21, 60)}, series(), n_trials=5)


def test_bad_market_raises():
    with pytest.raises(ValueError):
        emsl.tune(SmaCross, SPACE, series(), market="options", n_trials=5)
...
def test_bad_direction_raises():
    with pytest.raises(ValueError):
        emsl.tune(SmaCross, SPACE, series(), direction="up", n_trials=5)


def test_empty_space_raises():
    with pytest.raises(ValueError):
        emsl.tune(SmaCross, {}, series(), n_trials=5)


def test_strategy_must_be_callable():
    with pytest.raises(TypeError):
        emsl.tune(object(), SPACE, series(), n_trials=5)
