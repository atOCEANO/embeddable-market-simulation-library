...
`dev/differential/batch_differential.py` is the same idea one tier up, over `Batch`, and it runs in the same stage. Every reinforcement learning number comes out of `reset_at_each`, `advance` and `snapshot`, and what stood behind those was a Rust test comparing the batch against the engine it is built from, which is worth having and shares every mistake the engine makes. Each env is compared against a reference started at that env's own offset, because an episode beginning part way into the series must still fund on the bars the series funds on ([ADRs 0002, 0017, 0018](Decisions.md)), and no test that starts every env at bar zero can see that rule at all.

It has already earned its place twice. It found the slot leak of [ADR 0079](Decisions.md), which had survived every test ever written against the engine because none of them thought to ask whether an order filled down to float dust still holds its slot; it surfaced as five separate disagreements that all reduced to one line. The batched half then found [ADR 0082](Decisions.md) on its first run, funding marked at a price past the liquidation fence.

**A differential is only as good as the fields it compares**, and 0082 is the lesson. The single-engine harness read position and equity, which is what the engine's own promises are about. A liquidation zeroes the equity on both sides, so a disagreement about what the account paid on the way there was invisible: three thousand cases over six seeds could not see it, and two hundred batched cases could, only because the batched comparison happened to include `funding_paid`. Both harnesses now compare it. When a harness agrees too easily, ask what it is not looking at.
